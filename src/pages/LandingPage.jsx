...
import { ChevronLeft, ChevronRight, Utensils, Clock, CreditCard, Smartphone, Search, Star, MapPin, Phone, Pizza } from 'lucide-react'
...
const LandingPage = () => {
  ...
  useEffect(() => {
    const scrollRestaurants = () => {
      if (restaurantsRef.current) {
        ...
        if (restaurantsRef.current.scrollLeft >= restaurantsRef.current.scrollWidth / 2) {
  ...
  const handleSearch =  () => {
    setIsSearching(true);
    setTimeout(async () => {
      const response = await fetch(`http://localhost:8080/usuario/localidad/${selectedCity}`)
      if(!response.ok){
        throw new Error("Error en la solicitud " + response.status);
      }
      const data = await response.json();
  ...
  const carouselItems = [
    {
      ...
      description: 'Transforma la experiencia de tus clientes con nuestros menús digitales fáciles de usar.'
    },
    {
      image: '/img/wpmenu2.jpg?height=400&width=800',
      title: 'Gestión Eficiente de Pedidos',
      description: 'Optimiza tus operaciones con nuestra plataforma de gestión de pedidos en tiempo real.'
    },
    {
      image: '/img/wpmenu3.jpg?height=400&width=800',
      title: 'Análisis de Datos Avanzados',
      description: 'Toma decisiones informadas con nuestras herramientas de análisis de datos para restaurantes.'
    }
  ]

  const benefits = [
    { icon: <Utensils className="w-12 h-12 text-blue-500" />, title: 'Menús Personalizables', description: 'Crea y actualiza fácilmente tus menús con nuestra intuitiva interfaz.' },
    { icon: <Clock className="w-12 h-12 text-blue-500" />, title: 'Ahorro de Tiempo', description: 'Reduce el tiempo de espera y mejora la eficiencia de tu servicio.' },
    { icon: <CreditCard className="w-12 h-12 text-blue-500" />, title: 'Aumento de Ventas', description: 'Impulsa tus ventas con recomendaciones personalizadas y promociones.' },
    { icon: <Smartphone className="w-12 h-12 text-blue-500" />, title: 'Acceso Móvil', description: 'Gestiona tu restaurante desde cualquier lugar con nuestra app móvil.' }
  ]

  const restaurants = [
    { name: 'La Trattoria', image: '/img/restaurant_logo.png?height=200&width=200', cuisine: 'Italiana' },
    { name: 'Sushi Zen', image: '/img/restaurant_logo.png?height=200&width=200', cuisine: 'Japonesa' },
    { name: 'El Asador', image: '/img/restaurant_logo.png?height=200&width=200', cuisine: 'Argentina' },
    { name: 'Le Petit Bistro', image: '/img/restaurant_logo.png?height=200&width=200', cuisine: 'Francesa' },
    { name: 'Taco Loco', image: '/img/restaurant_logo.png?height=200&width=200', cuisine: 'Mexicana' },
    { name: 'Curry House', image: '/img/restaurant_logo.png?height=200&width=200', cuisine: 'India' },
    { name: 'The Burger Joint', image: '/img/restaurant_logo.png?height=200&width=200', cuisine: 'Americana' },
    { name: 'Dim Sum Palace', image: '/img/restaurant_logo.png?height=200&width=200', cuisine: 'China' }
  ]
    ...
    <div className="min-h-screen flex flex-col">
              ...
              >
                ...
                <div className="absolute inset-0 bg-black bg-opacity-50 flex items-center justify-center">
                  <div className="text-center">
                    <h2 className="text-3xl sm:text-4xl font-bold mb-4">{item.title}</h2>
      ...
      <section className="py-16 bg-gray-100">
        <div className="container mx-auto px-6">
          <h2 className="text-3xl font-bold text-center mb-12">Beneficios de Nuestra Plataforma</h2>
      ...
      <section className="py-16  bg-blue-50">
        <div className="container mx-auto px-6">
          <h2 className="text-3xl font-bold text-center mb-12">Encuentra Restaurantes Cerca de Ti</h2>
            ...
            <div className="flex flex-col md:flex-row gap-4 mb-8">
              ...
              >
                ...
                {provinceList.map((province) => (
                  <option key={province} value={province.id}>{province.nombre}</option>
                ))}
              </select>
              <select
                value={selectedCity}
              ...
              >
                ...
                {selectedProvince && cityList.map((city) => (
      ...
      <section className="py-16">
        <div className="container mx-auto px-6">
          <h2 className="text-3xl font-bold text-center mb-12">Restaurantes Asociados</h2>
          <div 
            ref={restaurantsRef}
